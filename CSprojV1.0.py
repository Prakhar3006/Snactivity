...
class Login():
		...
		def getem(step):
			...
			if (step == 1):
				...
				if (count == 1):
					...
					var1 = ent1.get()
					s.starttls()
					s.login("goelprakhar200306@gmail.com", "snactivity2003")

					message = f"Account Recovery\nYour otp is {otp}"
					s.sendmail("goelprakhar200306@gmail.com", var1, message)
					s.quit()
					s.close()
	...
	def page1(self):
		...
		logo = Image.open("Snactivity.png")
...
class Homepage(Login):
	...
	def info(self):
		...
		var = StringVar()  
		msg = Message( top, text ="""About Us:\nWhat is Snactivity's Story? How did it all begin?\nAn avocational project created and excecuted with the help of Prakhar.
                        \nSnactivity is an online food ordering and delivery platform currently serving in India.
			\nThe innovative technology, large and nimble delivery service,and exceptional customer focus at Snactivity enables a host of benefits that includes lightning fast deliveries, live order tracking and no limit on order amount, 
			all while having the pleasure of enjoying your favourite meal wherever you'd like it from wherever you want.\n The Snactivity project was initiated on May 2020 to ensure no one goes hungry during the lockdown.
	...
	def FoodData(self):
		...
		pas=f.read()
		pas=pas[0:len(pas)-1]
		mydb = sql.connect(host="localhost", user="root",passwd='zyrendiB1#',charset="utf8")
		mycursor = mydb.cursor()
		try:
			mycursor.execute("CREATE Database Snactivity")
		except:
			pass
		finally:
			mycursor.execute("USE Snactivity")
	...
	def Database(self):
		...
		pas=pas[0:len(pas)-1]
		try:
			mydb = sql.connect(host="localhost", user="root",passwd='zyrendiB1#',database="Snactivity",charset="utf8")
		except:
			mydb = sql.connect(host="localhost", user="root",passwd='zyrendiB1#',charset="utf8")
		...
		def SetButtons():
			...
			Button(frame2, text="Exit", padx=15, pady=10, command=win3.destroy, bg="orange").place(x=350, y=170)
		...
		def Bill():
			...
			c.drawString(100,650,"Thanks for choosing Snactivity:),Bon Appetit!!!! ") 
			c.drawString(100,638,"Here are your order details:")  
			styles = getSampleStyleSheet()
			c.setFont("Courier",25)
			c.drawString(100,750,"INVOICE")  
			t3 = "Disclaimer:This is an acknowledgement of Delivery of Order and not an actual invoice.Details mentioned above including the menu prices and taxes(as applicable) are provided by the restaurant to Snactivity.Resposiblity of charging taxes lies with the Restaurant and Snactivity disclaims any liablity that may arise in this respect."
...
client = MongoClient("mongodb+srv://user2003:snactivity@instafood.tpliu.mongodb.net/myFirstDatabase?retryWrites=true&w=majority")
